...
const SearchResultCard = ({ data }) => {
	...
	const url = data && type === "album" ? `/album/${id}` : data && type === "playlist" ? `/playlist/${id}` : data && type === "song" ? null : null
	return (
		<>
			{
				url !== null ? <div onClick={() => {
					...
					<div className='mt-3 flex items-center w-11/12 p-2 rounded-lg bg-black bg-opacity-5 hover:bg-opacity-10 duration-150 cursor-pointer'>
						...
						<div className='ml-4'>
							...
							<p className='text-xs font-bold text-gray-500 capitalize w-48 text-ellipsis overflow-hidden whitespace-nowrap' dangerouslySetInnerHTML={{ __html: data.type }}></p>
						</div>
					</div>
				</div> :
					<div onClick={() => {
